...
class VariableObject : public Object {
 public:
  explicit VariableObject(PrimitiveSimpleType* type) : type_(type) {
      if (type_->IsClass()) {
          // fields as symbols
          ...
          for (auto& field : fields) {
              if (field.second->IsArray()) {
                  ...
                  fields_[field.first] =
                      std::make_shared<VariableObject>(dynamic_cast<PrimitiveSimpleType*>(field.second.get()));
              }
          }
      } else {
          value_ = 0;
  ...
  VariableObject(PrimitiveSimpleType* type, int value) : type_(type), value_(value) {}
